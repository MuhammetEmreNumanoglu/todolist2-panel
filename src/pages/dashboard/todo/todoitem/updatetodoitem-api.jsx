...
const requestParams = [
  ...
  { name: "completed", value: false, type: "Boolean" },
...
export default function TodoUpdateTodoItemApiPage() {
  const [view, setView] = useState("Table");
  const [updatedTodoitem, setUpdatedTodoitem] = useState(null);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      const response = await todoAxios.patch(
        todoEndpoints.todoItem.updateTodoItem.replace(
          ":todoItemId",
          inputTodoItemId,
        ),
        data,
      ...
      setError(null);
      setUpdatedTodoitem(null);
      reset();
      console.info("RESPONSE", response);
      setUpdatedTodoitem(response.data.todoItem);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      {(updatedTodoitem || error) && (
        <Box paddingTop="2rem">
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={updatedTodoitem} error={error} />
            ) : (
              <JsonResponse content={updatedTodoitem || error} />
            )}
          </Box>
        </Box>
      )}
